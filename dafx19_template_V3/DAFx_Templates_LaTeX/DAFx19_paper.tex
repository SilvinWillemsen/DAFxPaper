...
\begin{figure}[h]
    ...
    \def\startY{0};
    \def\horShift{0.7};
    \foreach \drawing in {0, 1}
    {
        \node(stringText) at (0, -\drawing * 2) {string};
        \node(bowText) at (\drawing * \horShift, 1.2 - \drawing * 2) {bow};
        \begin{scope}
            \clip (-1.5,-0.2- \drawing * 2) rectangle (1.5,1.5);
            \draw (0,-\radius + 0.25 - \drawing * 2) circle(\radius);
            % \draw (-1.5,0) -- (1.5,0);
        \end{scope}
        \draw[-] (-2.5 + \drawing * \horShift,1 - \drawing * 2) -- (2.5 + \drawing * \horShift, 1 - \drawing * 2);
        \foreach \springNo in {0, ..., 5}
        {
            % Calculate springlength
            ...
            \pgfmathsetmacro{\spacing}{\springLength / 6};
            \pgfmathsetmacro{\invAngle}{\calcSpace/\horShift}
            \pgfmathsetmacro{\rot}{\drawing * (270+(atan(\invAngle)))};
            \pgfmathsetmacro{\angle}{atan(\invAngle)};
            % \node (nodeTest\springNo) at (\springNo*1.1-2, -3 - 1 * \drawing + 0.3 * \springNo) {\springLength};
            
            \begin{scope}[shift={(\startX + \drawing * \horShift,1 - \drawing * 2)}]
            % \pgfmathsetmacro{\startX}{\springNo  * 0.5 - 1.25};
            \pgfmathsetmacro{\xWidth}{0.1};
            \draw[-, rotate = \rot] (0, \startY) -- (0, \startY - \spacing * 0.5);
            \draw[-, rotate = \rot] (0, \startY - \spacing * 0.5) -- (\xWidth, \startY - \spacing);
            
            \def\Y{\startY-\spacing}
            \foreach \idx in {1,...,4}
            {
