...
\begin{tikzpicture}
    % \draw[dashed] (-10 0) -- (10 0)
     \node [circle, draw=black, fill=white, inner sep=0pt,minimum size=5pt](I) at (2.5, 9) {};
    \foreach \id in {1,...,4}
    {
        \node [circle, draw=black, fill=white, inner sep=0pt,minimum size=5pt](a\id) at (\id, 10) {};
        \node [circle, fill=black, inner sep=0pt,minimum size=2pt](b\id) at (\id, 9.5) {};
        \draw [-] (a\id) edge (b\id);
        \draw [->] (b\id) edge (I);
    }
