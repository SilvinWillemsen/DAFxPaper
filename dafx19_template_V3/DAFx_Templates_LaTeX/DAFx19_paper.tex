...
\begin{subequations}\label{eq:approximations}
    \begin{align}
        ...
        u_{txx} &\approx 
        \begin{aligned}[t]\delta_{t-}\delta_{xx} =& \; \frac{1}{hk^2}\big(u_{l+1}^n - 2u_l^2 + u_{l-1}^n \\
...
\begin{equation}
    \delta_{tt}u_l^n = c^2\delta_{xx}u_l^n-\kappa^2\delta_{xxxx}u_l^n-2\sigma_0\delta_t u_l^n + 2\sigma_1 \delta_{txx}u_l^n
\end{equation}
adding onto this
