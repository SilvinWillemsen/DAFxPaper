...
\end{equation}
and the aforementioned average bristle displacement $z$. In the implementation, $v_\text{B}$ will a user-controlled. In the following we will use the definitions found in \cite{Dupont2002}. The bowing function itself is defined as:
...
\end{equation}
with Stribeck velocity $v_\text{s}$ (in m/s), coulomb force $f_\text{c} = f_\text{N}\mu_\text{c}$ and stiction force $f_\text{s} = f_\text{N}\mu_\text{s}$ (both in N). Here $f_\text{N}$ is the normal force (in N) and $\mu_\text{c}$ and $\mu_\text{s}$ are the dynamic and static friction coefficient respectively. Like $v_\text{B}$, $f_\text{N}$ will be user-controlled in the implementation. Furthermore, the adhesion map between the bow and the string is defined as
...
\begin{figure}[h]
  ...
  \path[every node/.style={font=\sffamily\small}]
    ...
    % \end{scope}
  \end{tikzpicture}
  \caption{\it Hysteresis loop.\label{fig:hysteresis}}
\end{figure}
